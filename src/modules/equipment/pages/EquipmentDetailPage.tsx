    ...
    if (!equipment) {
        return (
            ...
            >
                ...
                <p className="text-[14px] text-muted-foreground">Nem található a keresett eszköz.</p>
                <button
    ...
    return (
        ...
        >
            ...
            <div className="grid gap-5 md:grid-cols-3">
                ...
                >
                    {/* Állapot csík */}
                    <div className={`h-[3px] w-full ${equipment.is_active ? 'bg-gradient-to-r from-accent-dark/50 to-transparent' : 'bg-gradient-to-r from-status-critical/40 to-transparent'}`} />

                    <div className="p-6 text-center space-y-4">
                        {/* Nagy eszköz ikon */}
                        <motion.div
                            whileHover={{ scale: 1.04 }}
                            className="w-20 h-20 rounded-full bg-gradient-to-br from-accent-dark to-accent-base flex items-center justify-center mx-auto"
                            style={{ boxShadow: '0 4px 16px -4px rgba(0,0,0,0.25)' }}
                        >
                            <Wrench className="w-9 h-9 text-white" strokeWidth={1.8} />
                        </motion.div>
                        ...
                        {/* Státusz badge */}
                        <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11.5px] font-semibold ${equipment.is_active ? 'status-ok' : 'status-critical'
                            }`}>
                            <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${equipment.is_active ? 'bg-status-ok' : 'bg-status-critical'}`} />
                            {equipment.is_active ? 'Aktív státusz' : 'Inaktív'}
                        </div>
                            ...
                            <div className="pt-4 border-t text-left"
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[13px] font-semibold text-text-primary">
                                        {equipment.responsible_user.full_name}
                                    </span>
                                </div>
                            </div>
                        )}
                ...
                <div className="md:col-span-2 space-y-5">
                    ...
                    >
                        ...
                        <div className="p-6">
                            {Object.keys(equipment.field_values || {}).length === 0 ? (
                                ...
                                <div className="grid gap-4 sm:grid-cols-2">
                                    {Object.entries(equipment.field_values || {}).map(([key, value], i) => {
                    ...
                    >
                        ...
                        <div className="p-6">
                            <EquipmentIncidentsList equipmentId={equipment.id} />
                        </div>
                    </motion.div>

                    {/* Dokumentumok kártya */}
                    <EquipmentDocs equipmentId={equipment.id} photos={equipment.photos || []} onRefresh={refetch} />
                </div>
            </div>
...
function EquipmentDocs({ equipmentId, photos, onRefresh }: { equipmentId: string, photos: Photo[], onRefresh: () => void }) {
    const { user } = useAuth();
    ...
    const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            // 1. Upload to Drive
            const driveFile = await googleStorage.uploadFile(file, 'Equipment', 'equipment');
            ...
            const { error } = await supabase
                ...
                .insert({
                    ...
                    filename: file.name,
                    description: 'Feltöltve az Eszköz adatlapról',
                    uploaded_by: user?.id
                });
...
function EquipmentIncidentsList({ equipmentId }: { equipmentId: string }) {
    const { incidents, isLoading } = useIncidents(equipmentId);
