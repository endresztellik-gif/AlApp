...
export function EquipmentForm({ initialData, onSave, onCancel, isOpen }: EquipmentFormProps) {
    const { entityTypes } = useEntityTypesAdmin();
    // Filter for equipment only
    const equipmentTypes = entityTypes.filter(t => t.module === 'equipment');

    const defaultTypeId = equipmentTypes.length > 0 ? equipmentTypes[0].id : null;
    ...
    }, [initialData, defaultTypeId, selectedTypeId]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!window.confirm("Biztosan menteni szeretnéd az eszköz adatait?")) {
            return;
        }
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            ...
                            <div className="sm:col-span-2">
                                <label className="text-xs font-medium text-muted-foreground mb-1 block">Megnevezés *</label>
                                <input
                                    type="text"
                                    required
