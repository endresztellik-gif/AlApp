-- 1. CHECK constraint DROP először (hogy az UPDATE átmenjen)
ALTER TABLE public.user_profiles
  DROP CONSTRAINT IF EXISTS user_profiles_role_check;

-- 2. Meglévő 'reader' userek frissítése
...
CREATE POLICY "vezeto_update_own_entities" ON public.entities
  ...
  USING (
    get_user_role() = 'vezető' AND (
      ...
      EXISTS (SELECT 1 FROM entity_user_links WHERE entity_id = entities.id AND user_id = auth.uid())
    )
  )
  WITH CHECK (
    get_user_role() = 'vezető' AND (
...
CREATE POLICY "vezeto_insert_own_photos" ON public.photos
  FOR INSERT TO authenticated
  WITH CHECK (
    get_user_role() = 'vezető' AND uploaded_by = auth.uid() AND
    EXISTS (
      SELECT 1 FROM entities e
      LEFT JOIN entity_user_links eul ON eul.entity_id = e.id
...
CREATE POLICY "vezeto_insert_incidents" ON public.incidents
  FOR INSERT TO authenticated
  WITH CHECK (get_user_role() = 'vezető' AND reported_by = auth.uid());

-- 9. water_facilities policies
...
CREATE POLICY "vezeto_select_water_facilities" ON public.water_facilities
  FOR SELECT TO authenticated USING (get_user_role() = 'vezető');
CREATE POLICY "vezeto_insert_water_facilities" ON public.water_facilities
  FOR INSERT TO authenticated
  WITH CHECK (get_user_role() = 'vezető' AND created_by = auth.uid());
CREATE POLICY "vezeto_update_own_water_facilities" ON public.water_facilities
  FOR UPDATE TO authenticated
  USING (get_user_role() = 'vezető' AND created_by = auth.uid())
  WITH CHECK (get_user_role() = 'vezető' AND created_by = auth.uid());

-- 10. water_facility_photos policies
...
CREATE POLICY "vezeto_insert_own_wf_photos" ON public.water_facility_photos
  ...
  WITH CHECK (
    get_user_role() = 'vezető' AND
    EXISTS (SELECT 1 FROM water_facilities wf WHERE wf.id = water_facility_photos.facility_id AND wf.created_by = auth.uid())
  );

-- 11. water_facility_documents policies
...
CREATE POLICY "vezeto_insert_own_wf_docs" ON public.water_facility_documents
  ...
  WITH CHECK (
    get_user_role() = 'vezető' AND uploaded_by = auth.uid() AND
    EXISTS (SELECT 1 FROM water_facilities wf WHERE wf.id = water_facility_documents.facility_id AND wf.created_by = auth.uid())
  );

-- 12. maintenance_logs policies
...
CREATE POLICY "vezeto_insert_own_maintenance_logs" ON public.maintenance_logs
  FOR INSERT TO authenticated
  WITH CHECK (
    get_user_role() = 'vezető' AND user_id = auth.uid() AND
    EXISTS (
      SELECT 1 FROM entities e
      LEFT JOIN entity_user_links eul ON eul.entity_id = e.id
...
-- 13. vehicle_checklists policies
...
-- 14. equipment_checkouts UPDATE policy
...
CREATE POLICY "checkout_update_own_or_admin" ON public.equipment_checkouts
  ...
  USING (
    ...
    OR EXISTS (
      ...
      WHERE id = auth.uid() AND role IN ('admin', 'vezető')
    )
  );
...
CREATE POLICY "admin_vezeto_upload_wf_docs" ON storage.objects
  FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'water_facility_documents' AND (get_user_role() = 'admin' OR get_user_role() = 'vezető'));
CREATE POLICY "admin_vezeto_update_wf_docs" ON storage.objects
  FOR UPDATE TO authenticated
  USING (bucket_id = 'water_facility_documents' AND (get_user_role() = 'admin' OR get_user_role() = 'vezető'));
...
CREATE POLICY "admin_vezeto_upload_wf_images" ON storage.objects
  FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'water_facility_images' AND (get_user_role() = 'admin' OR get_user_role() = 'vezető'));
CREATE POLICY "admin_vezeto_update_wf_images" ON storage.objects
  FOR UPDATE TO authenticated
  USING (bucket_id = 'water_facility_images' AND (get_user_role() = 'admin' OR get_user_role() = 'vezető'));
